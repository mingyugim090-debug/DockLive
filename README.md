...
## 예시
```
"지역구(자치구)별로 공급 세대수를 집계해서 새 시트에 표로 만들어줘"
"임대 유형(공공임대/행복주택 등)별 개수를 파이 차트로 그려줘"
"모집공고일이 이번 달 이후인 항목만 필터링해서 별도 시트로 정리해줘"
"보증금과 임대료를 숫자로 인식해서 자치구별 평균 임대료를 막대그래프로 만들어줘"
"이 목록에서 강남구, 서초구, 송파구만 골라서 새 파일로 정리해줘"
"전체 목록에 조건부 서식으로 보증금 5000만원 이하인 행에 색칠해줘"
