...
MATCH_PROMPT = """아래 공고 분석 결과와 회사/팀 프로필을 비교해 지원 적합도를 JSON으로 평가하세요.
...
def evaluate_match(analysis: dict, company_profile: CompanyProfile) -> dict:
    """Evaluate announcement fit for a company profile."""
    if not settings.OPENAI_API_KEY:
        return _mock_match_report(analysis, company_profile)
...
def _mock_match_report(analysis: dict, company_profile: CompanyProfile) -> dict:
    ...
    return {
        "score": score,
        "verdict": "기본 요건은 검토 가능하지만, 세부 자격은 공고 원문과 회사 정보를 더 확인해야 합니다.",
        "signals": [
            {
                "label": "지원 자격",
                "status": "unknown",
                "detail": "회사/팀 정보와 공고의 자격 조건을 추가 대조해야 합니다.",
            },
            {
                "label": "작성 가능성",
                "status": "match" if has_profile else "unknown",
                "detail": "프로필 정보가 있으면 사업계획서 초안에 바로 반영할 수 있습니다.",
            },
        ],
        "missing_inputs": ["사업자 소재지", "업력", "최근 매출/고용 현황", "지원 제외 이력"],
        "recommended_next_steps": ["필수 제출 서류를 확인하세요.", "불확실한 자격 조건을 담당 기관에 확인하세요."],
    }
