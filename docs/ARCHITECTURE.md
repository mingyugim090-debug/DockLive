...
LiveDock는 Next.js 프론트엔드와 FastAPI 백엔드로 구성됩니다. Supabase는 v1의 목표 persistence/auth/storage 플랫폼이며, Redis와 in-memory cache는 임시 fallback입니다.
...
1. 사용자가 PDF, URL, 텍스트 중 하나를 입력한다.
2. 백엔드가 텍스트를 추출하거나 URL 본문을 수집한다.
3. Analysis Agent가 공고 요건을 JSON으로 추출한다.
4. 백엔드가 `AnalysisResult`를 검증하고 D-day, fallback section을 보강한다.
5. Workflow session을 생성한다.
6. 프론트엔드가 필수 입력 필드를 보여준다.
...
| `POST` | `/api/workflow/{id}/draft` | 섹션별 초안 생성 |
...
- In-memory cache: 로컬 개발용 fallback

Production에서는 in-memory cache만으로 사용자 workflow를 유지하지 않습니다.

## HWPX Export Strategy

LiveDock는 `jkf87/hwpx-skill` 방향을 따른다.

- Markdown/Text -> HWPX는 HWPX toolchain을 통해 생성한다.
- 생성 후 `fix_namespaces.py`를 반드시 실행한다.
- `validate.py`로 구조 검증을 통과해야 한다.
- 복잡한 HWPX 양식 파일이 주어지면 XML 재작성보다 form cloning을 우선한다.
