...
LiveDock은 Next.js 프론트엔드와 FastAPI 백엔드로 구성됩니다. Supabase는 v1 목표 persistence/auth/storage 플랫폼이며, Redis 또는 in-memory cache는 로컬 개발과 임시 fallback입니다.
...
4. 백엔드는 `AnalysisResult`를 검증하고 D-day와 fallback section을 보강한다.
5. Workflow session을 생성한다.
6. 프론트엔드는 필수 사용자 입력 필드를 보여 준다.
7. Draft Agent가 섹션별 초안을 생성한다.
8. 사용자가 피드백을 저장하고 수정 요청을 보낸다.
9. 사용자가 확인하면 최종 문서를 생성한다.
10. HTML export를 기본 제공하고, HWPX toolchain 설정 시 HWPX export와 템플릿 클로닝을 제공한다.
...
| `GET` | `/api/workflow/{id}/export/hwpx` | Markdown 기반 HWPX export |
| `POST` | `/api/workflow/{id}/export/hwpx/template` | 업로드 HWPX 양식 클로닝 export |

## Key Contracts

- `AnalysisResult`: 공고 분석 결과, 제출서류, 작성 항목, 불확실 항목, 원문 근거를 포함한다.
- `WorkflowSession`: 분석 결과, 사용자 입력, 초안, 최종 문서 상태를 포함한다.
- `DraftSection`: 섹션별 초안과 `confirmation_required`를 포함한다.
...
Production에서는 in-memory cache만으로 workflow를 유지하면 안 됩니다.
...
- 복잡한 공식 HWPX 양식은 새 XML 생성보다 form cloning을 우선한다.
- `.hwp` 입력은 HWPX로 변환한 뒤 처리한다.

현재 API는 optional toolchain 방식입니다. `HWPX_EXPORT_ENABLED=true`와 `HWPX_SKILL_DIR`가 설정되어야 HWPX endpoint가 동작합니다.
