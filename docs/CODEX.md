...
이 문서는 Codex가 LiveDock를 개발할 때 따라야 할 실무 규칙입니다.
...
4. 한글 문자열이 깨진 파일을 만지면 해당 영역을 정상 한국어로 복구한다.

## Agent 품질 규칙

- 원문에 없는 핵심 사실을 만들지 않는다.
- 불확실한 정보는 `uncertain_fields`와 `confirmation_required`에 남긴다.
- 초안은 공고 분석 결과와 사용자 입력만 근거로 쓴다.
- 최종 문서 생성 전 사용자 확인 단계를 유지한다.
- 분석 결과에는 가능한 한 `source_evidence`를 포함한다.
...
- XML run을 직접 덮어써서 서식을 망가뜨리지 않는다.
