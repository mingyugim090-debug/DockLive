...
# ─── v2: 네이티브 차트 / 표 ──────────────────────────────────────────────
_CHART_TYPES = {"line", "column_clustered", "bar_clustered", "pie", "xy_scatter", "area"}


def create_chart(sheet: str, data_range: str, chart_type: str,
                 title: str, anchor_cell: str = "E2") -> dict:
    s, e = _require_book()
    if e:
        return e
    sh, e = _get_sheet(s, sheet)
    ...
    if chart_type not in _CHART_TYPES:
        return _err(f"지원하지 않는 chart_type: {chart_type}. 가능: {sorted(_CHART_TYPES)}")
    try:
        anchor = sh.range(anchor_cell)
        chart = sh.charts.add(left=anchor.left, top=anchor.top, width=420, height=260)
        chart.set_source_data(sh.range(data_range))
        chart.chart_type = chart_type
        try:
            chart.api[1].HasTitle = True            # WINDOWS-VERIFY: COM 차트 제목
            chart.api[1].ChartTitle.Text = title
        except Exception:
            pass  # 제목 실패는 치명적이지 않음
        return _ok(f"'{title}' {chart_type} 차트 생성 완료 ({sheet}!{anchor_cell} 위치, 원본 {data_range})")
    except Exception as ex:
        return _err(f"차트 생성 실패: {ex}")


def create_table(sheet: str, range: str, table_name: str) -> dict:  # noqa: A002
    s, e = _require_book()
    if e:
        return e
    sh, e = _get_sheet(s, sheet)
    if e:
        return e
    try:
        existing = [t.name for t in sh.tables]
        if table_name in existing:
            return _err(f"표 이름 '{table_name}' 이미 존재. 기존 표: {existing}")
        sh.tables.add(source=sh.range(range), name=table_name)
        return _ok(f"표 '{table_name}' 생성 완료 ({sheet}!{range}, 자동필터·줄무늬 적용)")
    except Exception as ex:
        return _err(f"표 생성 실패 (범위에 기존 표가 겹치는지 확인): {ex}")
