...
## 5. 한글 프로세스 (`src/tools/hwp_tools.py`)
- HwpCtrl COM (`HWPFrame.HwpObject`) + `XHwpWindows.Item(0).Visible = True` → 실시간 창
- 양식 채우기 우선순위: 누름틀(PutFieldText) > 찾아바꾸기 > 커서 삽입 (HWP_COM_GUIDE §2)
- hwp_open의 반환값에 필드 목록 포함 — Excel의 read_range에 해당하는 "구조 읽기"

## 6. 시각화 라우팅
| 대상 | 방법 | 도구 |
|---|---|---|
| Excel 차트 | 네이티브 COM 차트 (데이터 연동) | create_chart |
| Excel 표 | ListObject (자동필터·서식) | create_table |
| 한글 차트/그래프 | matplotlib PNG → 그림 삽입 | render_chart_image → hwp_insert_image |
| 한글 표 | TableCreate 액션 + 셀 순회 | hwp_insert_table |

## 검증 체계 (Codex용)
- Claude Code의 Stop hook 대신 `scripts/verify.py`가 게이트.
  AGENTS.md 규칙: 매 태스크 완료 전 verify.py 전체 통과 필수.
- COM 실동작은 Windows에서만 검증 가능 → `# WINDOWS-VERIFY:` 주석이 검증 포인트 목록.

---
# Phase 9: 무결성 채점 파이프라인

```
corpus/forms/(원본, 보호됨) ─→ extract(슬롯 스키마) ─→ fill(결정적 더미, 헤드리스)
     │                                                        │
     └────────────── 원본 vs 채움본 비교 ←────────────────────┘
                          │
                 checks C1~C5 (형식별)
                          │
        스코어카드: 통과율 % (workspace/reports/) ─→ verify.py 게이트
```

원칙: 채점기는 런타임 에이전트와 독립적인 헤드리스 경로를 갖는다 (COM 불필요 →
CI에서 실행 가능). COM 경로의 결과물도 같은 채점기로 교차 검증한다 (9c).
