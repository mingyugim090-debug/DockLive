...
// ---- Trait definitions for HTML fallback preview ----
...
// ---- HTML fallback rendering components ----
...
function DocumentModelFallback({ model, fileName }: { model: HwpxDocumentModel; fileName: string }) {
  return (
    <article className="max-h-[72vh] w-full overflow-y-auto border border-[#C9D2CD] bg-white px-8 py-10 text-[#202833] shadow-[0_28px_80px_rgba(36,49,45,0.18)] sm:px-12">
...
function HtmlFallback({ template }: { template: NoticeTemplate }) {
  const { sample } = template;
  const trait = getTrait(template);
  return (
    <article className="max-h-[72vh] w-full overflow-y-auto border border-[#C9D2CD] bg-white px-8 py-9 text-[#202833] shadow-[0_28px_80px_rgba(36,49,45,0.18)]">
...
type PdfStatus = 'loading' | 'ready' | 'error' | 'no-pdf';

export function HwpxSamplePreview({ template }: { template: SampleTemplate }) {
  const hasPdf = Boolean(template.previewPdfPath);
  const [pdfStatus, setPdfStatus] = useState<PdfStatus>(hasPdf ? 'loading' : 'no-pdf');

  const showPdf = hasPdf && pdfStatus !== 'error' && pdfStatus !== 'no-pdf';

  return (
    <div className="mx-auto w-full max-w-[620px]">
      {/* File tab bar */}
      <div className="flex items-center gap-2 rounded-t-md border border-b-0 border-[#D5DDD8] bg-[#EEF3F0] px-4 py-2">
        <span className="text-[10px] text-[#9CA3AF]">HWPX</span>
      ...
      {showPdf ? (
        <div className="relative border border-[#C9D2CD]">
          {pdfStatus === 'loading' && (
            <div className="absolute inset-0 z-10 flex min-h-[300px] items-center justify-center bg-white">
              <p className="text-sm text-[#9CA3AF]">PDF 미리보기 불러오는 중...</p>
            </div>
          )}
          <iframe
            src={template.previewPdfPath}
        ...
        <>
          {template.sample.documentModel ? (
            <DocumentModelFallback model={template.sample.documentModel} fileName={template.fileName} />
          ) : (
            <HtmlFallback template={template} />
          )}
        </>
      )}
    </div>
